// brute force would be taking an bool array of the size of the nums as... then ans can be between 
//[1 - nums.size()+1]
...
/*
    ...
    1. remove all the negative,zero, any number greater than length(nums) and keep all the req number in 
        HashTable of size size+1
    ...
    4. finally iterate through the HashTable and find the first indx with -1 return indx. else return size+1at 
    ...
    2. iterate the second time and this time we want to search each element in the resulting set[1 - size+1] at 
        O(1) time 
        so... we will do indx = element-1 and set element at this indxto negative kepping the magnitude same.. 
        so that we know this element is present.
        #case-1: 
    ...
    3. In the final iteration we will run a loop over our solution set considering each as element and finding 
        indx = element-1
